...
def main():
    ...
    company = st.selectbox('Brand Name',['Audi', 'BMW', 'Chevrolet', 'Datsun', 'Fiat', 'Force', 'Ford',
       'Hindustan', 'Honda', 'Hyundai', 'Jaguar', 'Jeep', 'Land',
       'Mahindra', 'Maruti', 'Mercedes', 'Mini', 'Mitsubishi', 'Nissan',
       'Renault', 'Skoda', 'Tata', 'Toyota', 'Volkswagen', 'Volvo'])
    years = range(2000, 2024)
    year = st.selectbox('Year:', years)
    kms_driven = st.slider('Kilometer Driven:', min_value=0, max_value=50000, step=100)
    fuel_type = st.selectbox('Fuel Type', ['Petrol', 'Diesel','LPG'])

    # Predict button
    if st.button('Predict'):
        result = predict_price(name, company, year, kms_driven, fuel_type)
